 ...
 */
export const syncSmspvaMessages = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const phoneNumber of phoneNumbers) {
      try {
        const syncResult = await syncSingleSmspvaNumber(phoneNumber.rent_id!, phoneNumber.id);
 ...
 */
export const syncSingleSmspvaNumberEndpoint = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await syncSingleSmspvaNumber(rentId, phoneNumber.id);
 ...
 */
export const syncSingleSmspvaNumber = async (rentId: string, phoneNumberId: string) => {
  try {
    console.log(`[SMSPVA-SYNC] Starting sync for rent ID: ${rentId}`);

    // Get messages from SMSPVA Rental Messages API
    const apiResponse = await smspvaService.getRentalMessages(rentId);
