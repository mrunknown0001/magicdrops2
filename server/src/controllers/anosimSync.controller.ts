...
import { AnosimBookingResolver } from '../services/anosimBookingResolver.service';

/**
 * Sync messages from all active Anosim phone numbers
 * Similar to SMSPVA sync controller but for Anosim
 */
export const syncAnosimMessages = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const phoneNumber of phoneNumbers) {
      try {
        ...
        const syncResult = await syncSingleAnosimNumber(bookingId, phoneNumber.id);
 ...
 */
export const syncSingleAnosimNumberEndpoint = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await syncSingleAnosimNumber(bookingId, phoneNumber.id);
 ...
 */
export const syncSingleAnosimNumber = async (bookingId: string, phoneNumberId: string) => {
  try {
    ...
    try {
      const testSms = await anosimService.getSms(bookingId);
      if (!Array.isArray(testSms) || testSms.length === 0) {
        ...
        const resolvedInfo = await AnosimBookingResolver.resolveBookingId(phoneRecord.phone_number);
        if (resolvedInfo && resolvedInfo.isActive) {
          ...
          await AnosimBookingResolver.updateDatabaseRecord(phoneNumberId, resolvedInfo);
        }
      }
    } catch (resolveError) {
    ...
    const smsResponse = await anosimService.getSms(actualBookingId);
