 ...
 */
export const syncPhoneNumbers = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
async function syncSmsActivateRentals() {
  ...
  const apiRentals = await smsActivateService.getRentList();
  ...
  if (typeof rentalValues === 'object' && !Array.isArray(rentalValues)) {
    ...
    for (const [rentId, data] of Object.entries(rentalValues)) {
      ...
      if (data !== null && typeof data === 'object') {
        ...
        phoneNumber = (data as any).phone || (data as any).number || '';
...
async function syncSmspvaRentals() {
  ...
  const apiRentals = await smspvaService.getRentList();
  ...
  const rentals: any[] = Array.isArray(apiRentals.rentals) ? apiRentals.rentals : [];
  const rentalData: RentalData[] = [];
 ...
 */
async function syncRentalsToDatabase(rentalData: RentalData[], provider: string) {
  ...
  const newRentals = rentalData.filter(rental => 
    !existingRentIds.has(rental.rent_id) && 
    rental.phone_number && 
...
async function syncGoGetSmsRentals() {
  ...
  const apiRentals = await gogetSmsService.getRentList();
