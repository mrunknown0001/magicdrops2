...
export class SchedulerService {
   ...
   */
  static startFollowUpScheduler(): void {
    if (this.isRunning) {
   ...
   */
  static stopFollowUpScheduler(): void {
    if (this.intervalId) {
   ...
   */
  static async processFollowUps(): Promise<void> {
    try {
      ...
      const results = await Promise.allSettled(
        pendingFollowUps.map(followUp => FollowUpService.sendFollowUpMessage(followUp.id))
      );
   ...
   */
  static getNextWorkingHours(): Date {
    const now = new Date();
   ...
   */
  static calculateReturnTime(managerStatus: any): Date {
    const now = new Date();
   ...
   */
  static async triggerFollowUpProcessing(): Promise<{
    processed: number;
    successful: number;
    failed: number;
  }> {
