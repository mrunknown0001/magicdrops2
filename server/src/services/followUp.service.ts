...
export class FollowUpService {
   ...
   */
  static async generateFollowUpMessage(followUp: FollowUp, userProfile: any): Promise<string> {
    try {
      ...
      const directAnswer = await this.generateDirectAnswer(
        followUp.user_question_summary,
        followUp.detected_topic,
        followUp.urgency_level
      );
...
${this.generateContextualClosing(followUp.urgency_level, followUp.detected_topic)}`;
   ...
   */
  static async sendFollowUpMessage(followUpId: string): Promise<boolean> {
    try {
   ...
   */
  static async markFollowUpSent(followUpId: string, messageId: string): Promise<void> {
    await supabase
      .from('chat_follow_ups')
      .update({
        status: 'sent',
   ...
   */
  static async markFollowUpFailed(followUpId: string, errorMessage: string): Promise<void> {
   ...
   */
  private static async analyzeUserMessage(message: string): Promise<{
    summary: string;
    topic: string;
...
Antwort: {
  ...
  "urgency": "normal"
      ...
      ], { model: 'gpt-4o-mini', temperature: 0.3 }) as any;
   ...
   */
  private static async generateDirectAnswer(
    ...
    try {
      ...
      const knowledgeContext = knowledgeArticles?.map(article => 
        `**${article.title}**: ${article.summary || article.content.substring(0, 150)}`
      ).join('\n\n') || '';
...
`;
      ...
      ], { model: 'gpt-4o-mini', temperature: 0.7, maxTokens: 200 }) as any;
   ...
   */
  private static getTimeContextualGreeting(followUp: FollowUp): string {
    const now = new Date();
   ...
   */
  private static generateContextualClosing(urgencyLevel: string, topic: string): string {
    ...
    return closings[topic as keyof typeof closings] || 'Falls noch Fragen aufkommen, melde dich gerne!';
  }

  /**
   * Fallback topic detection (if AI analysis fails)
   */
  private static detectTopicFallback(message: string): string {
    const text = message.toLowerCase();
    
    if (/kyc|verifizierung|dokument|ausweis|upload.*dokument/i.test(text)) return 'kyc';
    if (/funktioniert.*nicht|fehler|problem|lädt.*nicht|technisch/i.test(text)) return 'technical';
    if (/aufgabe.*abgelehnt|ablehnung|nicht.*akzeptiert/i.test(text)) return 'task_rejection';
    if (/zahlung|vergütung|geld|überweisung|bezahlung/i.test(text)) return 'payment';
    if (/aufgabe|task|schritt|anleitung|wie.*mache/i.test(text)) return 'task_help';
   ...
   */
  private static detectUrgencyFallback(message: string): 'low' | 'normal' | 'high' | 'urgent' {
   ...
   */
  static async getFollowUpAnalytics(timeframe = '7d'): Promise<{
   ...
   */
  static async cancelFollowUp(conversationId: string, reason = 'User received response'): Promise<void> {
    try {
      await supabase
        .from('chat_follow_ups')
        .update({
