...
import { sendApplicationConfirmationEmail } from './resend.service';
...
export class EmailSchedulerService {
   ...
   */
  static async getEmailDelaySettings(): Promise<EmailDelaySettings> {
    try {
   ...
   */
  async processEmailQueue(): Promise<EmailSchedulerStats> {
    const stats: EmailSchedulerStats = {
      processed: 0,
      sent: 0,
      failed: 0,
      skipped: 0
  ...
  private async processEmailItem(emailItem: EmailQueueItem): Promise<void> {
    ...
    switch (emailItem.email_type) {
      case 'application_confirmation':
        result = await sendApplicationConfirmationEmail(applicationData);
        break;
      default:
   ...
   */
  private async handleEmailFailure(emailItem: EmailQueueItem, error: any): Promise<void> {
   ...
   */
  static async handleApplicationEmail(
    applicationId: string,
    applicationData: any
  ): Promise<{ success: boolean; error?: string; messageId?: string; queueId?: string; immediate?: boolean }> {
    try {
